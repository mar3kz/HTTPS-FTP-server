...
// musi byt pro vice threadu, kazdy thread bude mit svoji a vsechno ostatni jako data pro session a stream
// prvni dva prvky jsou defaultne v ukazce, ale ja si tam chci dat svuj vlastni, proto se to dava na konec, pokud by nejake verze pouzivaly presny memory lazout te struktury, aby se to nerozbilo, aby to bylo kompatibilni
struct App_Context {
    SSL_CTX *ssl_ctx;
    struct event_base *evbase; // opaque pointer
    int connection_ac;
};

// typedef vytvori alias pro existujici datovy typ

struct Http2_Stream_Data {
    struct Http2_Stream_Data *prev, *next; // linked list, oba jak *prev, tak i *next je datoveho typu struct Http2_Stream_Data
    char *request_path;
    int32_t stream_id;
    int fd;
};
struct Http2_Stream_Data http2_stream_data;
...
// struct Http2_Session_Data http2_session_data;
...
struct App_Context **APP_CONTEXT; // SSL_CTX pro threads + struktura, ktera zjisti, jake jsou dostupne eventy pro socket // pole pointeru na struktury App_Context, aby kazdy thread mohl mit pristup ke sve strukture
struct Http2_Session_Data **HTTP2_SESSION_DT;
struct Http2_Stream_Data **HTTP2_STREAM_DT;
...
static void accept_callback(struct evconnlistener *listener, int fd, struct sockaddr *addr, int adrrlen, void *context_pointer);
static prepare_http2_session_data(struct App_Context *evbase_context, int connection_fd, struct sockaddr *client_address, int client_addresslen);
void session_send_data(struct Http2_Session_Data *session_data);
static void initialize_nghttp2_session(struct Http2_Session_Data *session_data);
...
// VSECHNY TY STRUKTURY HTTP2_SESSION_DATA A HTTP2_STREAM_DATA NEPOTREBUJI K TOMU, ABYCH MOHL NORMALNE PSAT KOD PRO NGHTTP2, TYTO STRUKTURY JSOU JENOM ZJEDNODUSENIM, ABYCH DATA NEMUSEL PRENASET PRES GLOBALNI PROMENNE APOD.

static void delete_http2_stream_data(struct Http2_Stream_Data *stream_data) {
    if (stream_data->fd != -1) {
...
static void delete_http2_session_data(struct Http2_Session_Data *session_data) {
    struct Http2_Stream_Data *stream_data;
    SSL *ssl = bufferevent_openssl_get(session_data->bev);
    ...
    nghttp2_session_end(session_data->session);

    // https://stackoverflow.com/questions/4894120/what-does-for-for-keyword-followed-by-a-pair-of-semicolons-in-parenthese
    // kdyz tam neni setup nebo advance (expression1 nebo expression3), tak to nic neznamena, jenom pokud to prostredni je true, tak to bude infinite loop/vykona se ten loop, pokud tam nic neni, tak je to automaticky true => loop forever
    for (stream_data = session_data->root.next; stream_data; ) {
        //kouka se jestli je stream_data != NULL
        http2_stream_data *next = stream_data->next;
        delete_http2_stream_data(stream_data);
        stream_data = next;
    }
    // free(session_data->app_context); // mozna tohle tam v tom kodu chybelo
...
static void writecb(struct bufferevent *bev, void *ptr) {
    ...
    if (evbuffer_get_length(bufferevent_get_output(bev)) > 0) {
        // mame evbuffer v buffereventu a my si ho vezmem, jestli je prazdny (pokud by byl, tak muzeme psat Bytes), pokud ne, tak se nejdrive musi vyprazdnit
        return; // ukonci se to nahle, nema cenu pokracovat
    }
    if (nghttp2_session_want_read(session_data->session) == 0 && nghttp2_session_want_write(session_data->session) == 0) {
        delete_http2_session_data(session_data);
        return;
    }
    session_send_data(session_data);
}

static void readcb(struct bufferevent *bev _U_, void *ptr) {
    // _U_ atribut, ktery rekne kompilatoru, ze pokud se nepouzije *bev, tak ze nechceme warning
    struct Http2_Session_Data *session_data = (struct Http2_Session_Data *)ptr;
    (void)ptr;
...
static nghttp2_ssize nghttp2_session_send_cb(nghttp2_session *session _U_, const uint8_t *data, size_t length, int flags, void *user_data) {
...
static int nghttp2_session_recv_cb(nghttp2_session *session, const nghttp2_frame *frame, void *user_data) {
    // nghttp2_frame je union, ktera reprezentuje jakykoliv frame
    ...
    switch (frame->hd.type) {
        ...
        case NGHTTP2_HEADERS:
            if (frame->hd.flags & NGHTTP2_FLAG_END_STREAM) {
                stream_data = nghttp2_session_set_stream_user_data(session, frame->hd.stream_id);
            }
...
static void nghttp2_session_stream_close_cb() {

}

static void nghttp2_session_header_cb() {

}

static void nghttp2_session_begin_headers_cb() {

}

static void initialize_nghttp2_session(struct Http2_Session_Data *session_data) {
    nghttp2_session_callbacks *callback_for_nghttp2_session; // promenna slouzici k tomu, ze "do ni" nacteme vsechny callbacky, potom ji predame hlavni strukture nghttp2 session, aby nghttp2 melo zpusob, jak vedet, co ma delat, kdyz dostane informace od nejakeho peera

    if (nghttp2_session_callbacks_new(&callback_for_nghttp2_session) != 0) { // inicialuzuje danou promennou
        handle_error_thread(pthread_self());
    }

    nghttp2_session_callbacks_set_send_callback(callback_for_nghttp2_session, nghttp2_session_send_cb); // kdyz se odeslou data => callback

    nghttp2_session_callback_set_on_frame_recv_callback(callback_for_nghttp2_session, nghttp2_session_recv_cb); // kdyz se prijmou data => callback

    nghttp2_session_callback_set_on_stream_close_callback(callback_for_nghttp2_session, nghttp2_session_stream_close_cb); // kdyz se zavre stream => callback

    nghttp2_session_callback_set_on_header_callback(callback_for_nghttp2_session, nghttp2_session_header_cb); // kdyz prijde frame, kde jsou value/name pary, u HTTP/1.1 je to ve stylu => Connection: close, v HTTP/2 je to ve stylu => :scheme: https, Connection: close, nejde dat v HTTP/2 a HTTP/3

    // http2 to ma tak, ze kdyz chce poslat nekolik tich ramcu (frame), tak ty frames jsou bud HEADERS nebo PUSH_PROMISE a protoze se pouziva HPACK, tak se nekdy ty requesty nevejdou do jednoho ramce (frame), takze se poslou za sebou, no a kdyz se zachyti, ze prichazeji ty HEADER nebo PUSH_PROMISE ramce (frame), tak se spusti tento callback a potom se muzou spustit ty callbacky pro jednotlive ramce (frame)
    nghttp2_session_callback_set_on_begin_headers_callback(callback_for_nghttp2_session, nghttp2_session_begin_headers_cb);

    nghttp2_session_server_new(&session_data->session, callback_for_nghttp2_session, session_data); // zkopiruje jmena tich funkci, ktere jsme dali jako callbacky a rekneme ngttp2, ze ho v tomto pripade chceme pouzivat jako server

    nghttp2_session_callbacks_del(callback_for_nghttp2_session); // uvolni prostredky => free()
}

static void eventcb(struct bufferevent *bev, short int events_flags, void *ptr) {
    // bev = bufferevent

    struct Http2_Session_Data *http2_session_data = (struct Http2_Session_Data *)ptr;

    if (events_flags & BEV_EVENT_CONNECTED) {
        // zkontroluje se jestli je nastaven bit (hodnota BEV_EVENT_CONNECTED) v event_flags = bitwise operator AND


        SSL *current_ssl_ctx = bufferevent_openssl_get_ssl(bev);
        unsigned char *selected_alpn_protocol;
        int selected_alpn_protocol_len;

        SSL_get0_next_proto_negotiated(current_ssl_ctx, &selected_alpn_protocol, &selected_alpn_protocol_len);

        if (selected_alpn_protocol_len != 2 || memcmp("h2", selected_alpn_protocol, 2) != 0) {
            fprintf(stderr, "SSL_get0_next_proto_negotiated zachytilo h2 nebylo domluveno alpn");
            handle_error_thread(pthread_self());
        }
...
static int session_recv(struct Http2_Session_Data *session_data) {
    struct evbuffer *evbuff = bufferevent_get_output(session_data->session);
    size_t length_of_data = evbuffer_get_length(evbuff);

    // evbuffer neni contiguous (sousedici) memory, je to skupina linked lists
    unsigned char *memory_evbuffer = evbuffer_pullup(evbuff, -1); // -1 = cely buffer bude contiguous (sousedici)

    nghttp2_ssize len = nghttp2_session_mem_recv2(session_data->session, memory_evbuffer, length_of_data);
    if (len < 0) {
        // je to mensi nez 0, protoze vsechny error enums jsou zaporne hodnoty
        fprintf(stderr, "nghttp2_session_mem_recv2() selhal");
        delete_http2_session_data(session_data);
        handle_error_thread(pthread_self());
    }
    if (evbuffer_drain(evbuff, length_of_data) == -1) { // libevent automaticky ty Bytes z toho evbuffer neodstrani, musime si to udelat samostatne
        fprintf(stderr, "evbuffer_drain() selhal")
        delete_http2_session_data(session_data);
        handle_error_thread(pthread_self());
    }
...
static void accept_callback(struct evconnlistener *listener, int fd, struct sockaddr *addr, int adrrlen, void *context_pointer, int connection) {
    /*
    jak zjistim, kde se predavaji data do accept callbacku z start_listen() =>``` create_http2_session_data```
    vyhledam evconnlistener_new() .c github, to mi vyjede listener.c, najdu si tam tu funkci ```evconnlistener_new()``` a koukam se na vsechny volane funkce a zjistim ze funkce ```event_assign()``` prirazuje funkci, ktera opravdu acceptuje connection a v te funkci je videt, ze vola callback ```cb()```, kde dava i adresu clienta
    https://github.com/libevent/libevent/blob/master/listener.c#L436
    */
    // int connection je moje vlastni promenna
    struct App_Context *app_context = (struct App_Context *)context_pointer;
    struct Http2_Session_Data *http2_session_data;
    (void)listener; // tento vyraz je pro compilator, protoze by vyhodil warning, ze jeden parametr jsme nepozili, proto (void)x; je vyraz, ktery rika, vse ok, jen nechci pouzit tento parametr

    http2_session_data = prepare_http2_session_data();
    bufferevent_setcb(http2_session_data->bev, readcb, writecb, eventcb, cbarg); // pokud bufferevent bude mit nejaky event, tak specifukujeme event callback, pokud read, tak read callback a pokud write, tak write callback
...
static prepare_http2_session_data(struct App_Context *evbase_context, int connection_fd, struct sockaddr *client_address, int client_addresslen, int connection) {
    // int connection je moje vlastni promenna
    int result_getnameinfo;
    char hostname[NI_MAXHOST]; // 1025 Bytes, defaultni hodnota, sjednocena hodnota, pro vsechny priprady
    struct Http2_Session_Data *http2_session_data; // i kdyz je to pointer, tak stejne na nej musim alokovat memory!!
    // SSL *current_ssl_settings = SSL_new(evbase_context->ssl_ctx); // davam do komentare, protoze ja mam "kovenci" takovou, ze ja nejdrive udelam tu current settings ssl instanci a potom ji vkladam do dalsich funkcich => vlozim sem jenom connection a tu strukturu si vytahnu
    SSL *current_ssl_ctx = CONNECTIONS_ARRAY[connection];

    http2_session_data = malloc(sizeof(Http2_Session_Data));
    if (http2_session_data == NULL) {
        handle_error_thread(pthread_self());
    }
    memset(http2_session_data, 0, sizeof(struct Http2_Session_Data));
    http2_session_data->app_ctx = evbase_context; // evbase = event base

    int nonzero_value = 1;
    // vyne Nagle algoritmus (cekani, nez v TCP segmentu bude dostatecne data) => muzeme posilat malinke packety i s malym obsahem dat
    if (setsockopt(connection_fd, IPPROTO_TCP, TCP_NODELAY, nonzero_value, sizeof(int)) != 0) {
        handle_error_thread(pthread_self());
    }

    // bufferevent je jakoby zpusob, jak ziskat event takovy, ze socket je ready na writing a reading a potom diky tomuto mechanismu cist/zapisovat ty data? v libevent
    // bufferevent_openssl_event_new() je tento bufferevent pro SSL/TLS
    // nastavuji options specifikujici chovani tohoto buffereventu
    http2_session_data->bev = bufferevent_openssl_event_new(evbase_context, connection_fd, current_ssl_ctx, BUFFEREVENT_SSL_ACCEPTING, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_THREADSAFE | BEV_OPT_DEFER_CALLBACKS); // kdyztak je tam jeste jedna option na unlockovani mutexu pro callbacky
    if (http2_session_data->bev == NULL) {
        handle_error_thread(pthread_self());
    }

    // evbuffery a buffereventy jsou defaultne disablenute
    if (bufferevent_enable(http2_session_data->bev, EV_READ | EV_WRITE) != 0) {
        handle_error_thread(pthread_self());
    }

    // protoze chceme jenom tu IP adresu, tak pouzijeme getnameinfo jenom pro to, aby nam vypsala tu IP adresu, nemuzeme rovnout sahat do te struktury, protoze se pouzije bud struktura pro IPv4 nebo pro IPv6 automaticky a museli bychom napsat velky switch/if statement, proto se pouziva getnameinfo
    result_getnameinfo = getnameinfo(client_address, client_addresslen, hostname, NI_MAXHOST, NULL, 0, NI_NUMERICHOST); // vezme IP adresu bez service (port)
    if (result_getnameinfo == 0) {
        http2_session_data->client_addr = strdup(hostname); // strdup to ulozi na heap, takze az zanikne hostname, tak to porad bude existovat
    }
    else {
        http2_session_data->client_addr = strdup("error_occured");
    }

    return http2_session_data;
}
...
int main()
{
    ...
    evthread_use_pthreads(); // je potreba dat do compilation 
    ...
    APP_CONTEXT = (struct App_Context **)calloc(MAX_THREADS, sizeof(struct App_Context *));
    HTTP2_SESSION_DT = (struct Http2_Session_Data **)calloc(MAX_THREADS, sizeof(struct Http2_Session_Data *));
    HTTP2_STREAM_DT = (struct Http2_Stream_Data **)calloc(MAX_THREADS, sizeof(strcut Http2_Stream_Data *));
    ...
    // nastavi flags pro socket, aby byl non blocking
    if (fcntl(http_socket, F_SETFL, O_NONBLOCK) == -1) {
        perror("fcntl() selhalo - http");
        exit(EXIT_FAILURE);
    }
   ...
   // webservery maji limit na to kolik mohou mit TCP spojeni v jednu dobu => je to 6-8 => toto ale znamena, ze muze webbrowser stahovat 6-8 souboru z jedne domeny, ale take zalezi na verzi HTTP, ale vetsinou ty browsery se snazi znovu pouzivat ty uz natolene TCP spojeni, takze 1 webbrowser, 1 stranka, 2 nebo vice oken otevrenych => 1 TCP spojeni => 1 TCP handshake
    ...
    // http_socket a jeho flags ovlivni, jak se bude chovat accept(), pokud je ten socket nonblocking, tak se koukne, jestli je tam nejake connections, pokud ne, tak to vyhodi EGAIN/EWOULDBLOCK, pokud je blocking, tak accept() ceka na connection nez se objevi
    if ( listen(http_socket, BACKLOG) == -1) {
        perror("listen() selhal - http");
        return EXIT_FAILURE;
    }
    
    // KDZYTAK ACCEPT() MUSIM UDELAT TAK, ABY TO BYLO NON BLOCKING, ALE POTOM ABY KDYBY NEBYLO CONNECTION, TAK ABY TO HNED NESPADLO!!
    ...
    for(;;) {
        ...
        printf("\n\nAVE CHRISTUS REX\n\n");
        // doporucuji precist https://istio-insider.mygraphql.com/en/latest/ch2-envoy/arch/event-driven/libevent.html
        ...
        struct event_base *base = event_base_new();// zkracene event je neco, co se stane file descriptoru (dostane signal, je pripraven na cteni/zapisovani...), event_base je kolekce tichto eventu, tato struktura ma svoji vlastni metodu na zjistovani, jake eventy se staly jako select(), poll(), epoll() apod. to se vybere samo

        
        pthread_mutex_unlock(&lock_thread_info); // odemceno
        void (*accept_callback_ptr)(struct evconnlistener *listener, int fd, struct sockaddr *addr, int adrrlen, void *context_pointer) = accept_callback; // muze byt static nebo i nemusi
        struct evconnlistener *listener = evconnlistener_new(base, accept_callback, accept_callback_ptr, BACKLOG) // struct evconnlistener je mechanismus, ktery za nas prijme TCP connection, evconnlistener_new() uz ocekava nas file descriptor, ale pokud bychom si to chteli zjednodusit jeste vice, tak muzeme pouzit evconnlistener_new_bind(), coz se postara i o ten socket
        ...
        if (CONNECTION == (MAX_THREADS - 1) ) {
            ...
            struct event_base **event_context_temp = realloc(APP_CONTEXT, sizeof(struct event_base *) * MAX_THREADS); // realloc() stary blok memory automaticky uvolni, nemusim to delat manualne!
            APP_CONTEXT = event_context_temp;
